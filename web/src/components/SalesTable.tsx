...
import { getSales, type SaleRead } from '../api/sale'

export function SalesTable() {
  const { getToken, isSignedIn } = useAuth()
  const { organization } = useOrganization()
  const [rows, setRows] = useState<SaleRead[]>([])
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-[#14141a]/80 p-4 shadow-xl shadow-black/50 backdrop-blur-xl">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Продажі</p>
